...
function StepContent({ step, formData, setFormData, consultType, motivoConsultaHistoria }) {
  switch (step) {
    case 0:
      return null;
      case 1:
        return (
          <Box sx={{ p: 2 }}>
            <Typography variant="h6" sx={{ mb: 2 }}>
              {consultType === 'control' ? 'Motivo de Consulta Original' : 'Motivo de Consulta'}
            </Typography>
            {consultType === 'control' ? (
              <Typography variant="body1" sx={{ mb: 2 }}>{motivoConsultaHistoria}</Typography>
            ) : (
              <TextField
                multiline
                rows={3}
            ...
            <Typography variant="h6" sx={{ mb: 2 }}>Signos Vitales</Typography>
            ...
            <Typography variant="h6" sx={{ mb: 2 }}>
              {consultType === 'control' ? '¿Presenta algún problema?' : 'Problema Actual'}
            </Typography>
            <TextField
              multiline
              rows={3}
              placeholder={consultType === 'control' ? '¿Presenta algún problema?' : 'Problema Actual'}
              fullWidth
              variant="outlined"
              value={formData.problemaActual}
              onChange={(e) => setFormData({ ...formData, problemaActual: e.target.value })}
            />
          </Box>
        );
    case 2:
      return (
        <Box sx={{ p: 1 }}>
          <Typography variant="body2" sx={{ textAlign: 'start', p: 1 }}>CP = CON EVIDENCIA DE PATOLOGÍA: MARCAR "X" Y DESCRIBIR ABAJO ANOTANDO EL NÚMERO Y LETRA SP = SIN EVIDENCIA DE PATOLOGÍA: MARCAR "X" Y NO DESCRIBIR</Typography>
    ...
    const loadPaciente = async () => {
      ...
      if (pacienteId) {
        try {
          ...
          setMotivoConsultaHistoria(historiaData.motivo_consulta_historia || '');
          setActiveStep(1);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (consultType !== 'control') {
        ...
        historiaFormData.append('motivo_consulta_historia', formData.motivoConsulta);
  ...
  return (
    <Box sx={{ display: 'flex', mt: 8, p: 1 }}>
      ...
      <Box sx={{ flexGrow: 1, p: 3, width: '100%' }}>
        {patientLoaded && (
          <Grid container sx={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', justifyContent: 'space-between', alignItems: 'center', mb: 2, boxShadow: 1, paddingRight: 1, paddingLeft: 1 }}>
            <Grid item sx={8}>
              <CuadroPaciente  isDeleteDisable={false} />
            </Grid>
            <Grid item sx={4}>
              <Box sx={{ mt: isMobile ? 2 : 0, display: 'flex', flexDirection: isMobile ? 'column' : 'row' }}>
              <Button variant="contained" color="error" onClick={handleCancel} sx={{ mr: isMobile ? 0 : 2, mb: isMobile ? 1 : 0 }}>
                  Cancelar
                </Button>
        ...
        <Stepper activeStep={activeStep} sx={{ mb: 2 }} orientation={isMobile ? 'vertical' : 'horizontal'}>
          {steps.filter((_, index) => consultType !== 'control' || (index !== 2 && index !== 4)).map((label) => (
            <Step key={label}>
        ...
        {saveSuccess ? (
          <Box sx={{ mt: 2, mb: 1, p: 2, backgroundColor: '#e8f5e9', borderRadius: 2 }}>
            <Typography variant="h6" sx={{ color: '#2e7d32', mb: 1 }}>
              ¡Atención médica registrada exitosamente!
            </Typography>
            <Typography variant="body1">
              Todos los pasos se han completado y la información ha sido guardada.
            </Typography>
            <Button variant="contained" color="primary" onClick={() => navigate('/fcc-atencion')} sx={{ mt: 2 }}>
        ...
        >
          <DialogTitle id="alert-dialog-title">{"¿Está seguro de guardar la atención médica?"}</DialogTitle>
          <DialogContent>
            <DialogContentText id="alert-dialog-description">
              Una vez guardada, no podrá modificar esta atención. Asegúrese de que todos los datos estén correctos.
            </DialogContentText>
          </DialogContent>
          <DialogActions>
